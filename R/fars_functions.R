...
#'
#' @examples
#' \dontrun{fars_read('daily_SPEC_2014.csv.bz2')}
#'
...
#'
#' @examples
#' \dontrun{fars_read_years(2010:2017)}
#'
...
#'
#' @examples
#' \dontrun{fars_summarize_years(2015:2016)}
#'
...
fars_summarize_years <- function(years) {
    ...
    dplyr::bind_rows(dat_list) %>% dplyr::group_by(year, MONTH) %>% dplyr::summarize(n = n()) %>% tidyr::spread(year,
        n)
}
...
#' @param year  \code{vector} of \code{numerical} value that converted to the year, used to specify filename
#'
